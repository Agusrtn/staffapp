...
// ================= CLEAR CONNECTED USERS (KEEP AGUSTINSON) =================
app.post("/admin/users/clear-connected", (req, res) => {
  const connected = Array.from(new Set(Array.from(connectedUsers.values())));
  const toRemove = connected.filter(u => u !== ADMIN_USER.user);

  // Remove logged-in users from users list
  const originalCount = users.length;
  users = users.filter(u => !toRemove.includes(u.user));

  saveData({users,tasks,messages,accessRequests});

  if(io) {
    // Force logout the removed users
    for (const [socketId, username] of connectedUsers.entries()) {
  ...
  res.json({success:true, removed: originalCount - users.length, removedUsers: toRemove});
});
